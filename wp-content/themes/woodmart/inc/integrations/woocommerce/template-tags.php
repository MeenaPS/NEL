...
if ( ! function_exists( 'woodmart_sticky_sidebar_button' ) ) {
	function woodmart_sticky_sidebar_button( $echo = true, $toolbar = false ) {
		...
		if ( strstr( $sidebar_class, 'col-lg-0' ) || woodmart_maintenance_page() || ( $sticky_toolbar && isset( $sticky_toolbar_fields['enabled']['sidebar'] ) && ! $toolbar ) ) {
			return;
		}
				...
				$links['register']['dropdown'] = '
					<div class="sub-menu-dropdown color-scheme-' . $color_scheme . '">
						<div class="login-dropdown-inner">
							<h3 class="login-title"><span>' . esc_html__('Sign in', 'woodmart') . '</span><a class="create-account-link" href="' . esc_url( $account_link ) . '?action=register">' . esc_html__('Create an Account', 'woodmart') . '</a>' . '</h3>
		...
		?>
			<div class="login-form-side">
				...
				<div class="create-account-question">
					...
					<a href="<?php echo esc_url( $account_link ); ?>?action=register" class="btn btn-style-link btn-color-primary create-account-button"><?php esc_html_e( 'Create an Account', 'woodmart' ); ?></a>
				</div>
			</div>
		<?php
	}
...
if ( ! function_exists( 'woodmart_my_account_navigation_endpoint_classes' ) ) {
	function woodmart_my_account_navigation_endpoint_classes( $classes, $endpoint ) {
		global $wp;
	...
	add_filter( 'woocommerce_account_menu_item_classes', 'woodmart_my_account_navigation_endpoint_classes', 15, 2 );
}
