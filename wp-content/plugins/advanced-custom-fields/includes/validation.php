...
function acf_validate_value( $value, $field, $input ) {
	...
	if( $field['required'] ) {
     ...
     /* Series custom field input validation */

	if($field['name']=="series" && !empty(($value))){
		if(!ctype_alnum($value)){
		$message = sprintf( __( '%s field accept only alphanumeric character', 'acf' ), $field['label'] );
		$valid = false;
	  }
	}

	/* manufactured year custom field input validation */
	
	if($field['name']=="manufactured_year" && !empty(($value))){

		$current_year = date("Y"); //get current year
		
		if (!preg_match('/^[0-9]*$/', $value)) {
	        $message = sprintf( __( '%s field accept only number', 'acf' ), $field['label'] );
			$valid = false;
        }
        
		if(preg_match('/^[0-9]*$/', $value) && strlen($value) < 4){
			$message = sprintf( __( 'please enter the correct year format, Eg:2001', 'acf' ), $field['label'] );
			$valid = false;
		}
	   //check if given year is > current year is show error
		if(preg_match('/^[0-9]*$/', $value) && ($value > $current_year)){
			$message = sprintf( __( '%s field cannot allow the future year', 'acf' ), $field['label'] );
			$valid = false;
		}
	}
