...
class WC_Admin_List_Table_Products extends WC_Admin_List_Table {
	...
	protected function render_is_in_stock_column() {
		...
		//customized the stock status condition on product dashboard view - 25-jan-19

		if($this->object->stock_status == 'instock'){
			$stock_html = '<mark class="instock">' . __( 'In stock', 'woocommerce' ) . '</mark>';
		}
		elseif($this->object->stock_status == 'onrequest'){
			$stock_html = '<mark class="onbackorder">' . __( 'On request', 'woocommerce' ) . '</mark>';
		}
		elseif($this->object->stock_status == 'onbackorder'){
